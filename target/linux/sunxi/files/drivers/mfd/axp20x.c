...
static const struct regmap_range axp858_writeable_ranges[] = {
	...
	regmap_reg_range(AXP858_OUTPUT_CONTROL1, AXP858_CPUSLDO_CTL),
...
static const struct mfd_cell axp858_self_working_cells[] = {
	{
		.name		= "axp221-pek",
...
static const struct mfd_cell axp858_cells[] = {
	{
		.name		= "axp221-pek",
		.num_resources	= ARRAY_SIZE(axp858_pek_resources),
		.resources	= axp858_pek_resources,
...
int axp20x_match_device(struct axp20x_dev *axp20x)
{
	...
	case AXP858_ID:
		if (of_property_read_bool(axp20x->dev->of_node,
					  "x-powers,self-working-mode") &&
		    axp20x->irq > 0) {
			axp20x->nr_cells = ARRAY_SIZE(axp858_self_working_cells);
			axp20x->cells = axp858_self_working_cells;
		} else {
